...
class Produk extends Connect{
    ...
    public function readProduk(){
        ...
        public function viewEachProduct($id_produk){
            $conn = $this->getConnection();
            $query = "SELECT jumlah_stok FROM produk WHERE id_produk= $id_produk";
            $result = $conn->query($query);
    ...
    public function addProduk($data){
        ...
        $stmt->bindParam(2,$jumlah_stok);
        $stmt->bindParam(3,$keterangan_produk);
        $stmt->bindParam(4,$gambar);
        $stmt->bindParam(5,$id_kategori);
        $stmt->execute();
        return true;
    }
